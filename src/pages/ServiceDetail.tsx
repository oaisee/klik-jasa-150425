    ...
    <div className="animate-fade-in pb-8">
      ...
      <div className="relative h-64">
        ...
        <div className="absolute bottom-4 right-4 flex space-x-2">
          {service.images.map((_, idx) => <button key={idx} className={`w-2 h-2 rounded-full ${idx === currentImage ? 'bg-white' : 'bg-white/50'}`} onClick={() => setCurrentImage(idx)} />)}
        </div>
      </div>
      ...
      <div className="px-4">
        ...
        {/* Description */}
        <div className="mt-6">
          <h2 className="text-lg font-semibold mb-2 text-center">Deskripsi</h2>
        ...
        <div className="mt-8">
          ...
          <div>
            {service.reviews.map((review, idx) => <ReviewItem key={idx} name={review.name} date={review.date} rating={review.rating} content={review.content} />)}
          </div>
        </div>
        
        {/* Action buttons */}
        <div className="mt-8 flex gap-3">
          <Button variant="outline" className="flex-1 flex items-center justify-center" onClick={handleChat}>
            <MessageCircle size={18} className="mr-2" />
            Chat
          </Button>
          <Button className="flex-1 bg-marketplace-primary hover:bg-marketplace-secondary" onClick={handleRequestBooking}>
            Pesan Layanan
          </Button>
        </div>
      </div>
